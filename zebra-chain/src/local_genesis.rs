            ...
            let secret_key = match options.seed {
                ...
                Some(seed) => {
                    ...
                    loop {
                        ...
                        counter = counter
                            .checked_add(1)
                            .expect("a valid key is found quickly");
                    }
                }
...
fn build_block(
    ...
    let coinbase_input = if height == Height(0) {
        ...
        // Upstream removed `Input::new_coinbase`; `data` is now the raw miner data (the
        // height is serialized separately). This data is short and non-empty, so it
        // satisfies the 2..=100 byte coinbase-script consensus rule without padding.
        transparent::Input::Coinbase {
            height,
            data: format!("kresko h={}", height.0).into_bytes(),
            sequence: 0,
        }
    };
